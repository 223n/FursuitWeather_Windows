...
`[Add]`、`[Mod]`、`[Fix]`、`[Del]`、`[Doc]`のいずれかで始めてください。

**この接頭辞がリリースノートの分類になります。**
ワークフローが接頭辞を読み、次のラベルを付けます。

| 接頭辞 | 付くラベル | リリースノートの見出し |
| ---- | ---- | ---- |
| `[Add]` | 機能追加 | 新しい機能 |
| `[Fix]` | バグ | 不具合の修正 |
| `[Doc]` | ドキュメント | 文書 |
| `[Mod]`と`[Del]` | 付きません | その他の変更 |

接頭辞が無いと「その他の変更」に入ります。
`破壊的変更`と`セキュリティ`は自動で付かないため、当てはまるときは手で付けてください。

...
アプリの中の案内はまだ実装しておらず、いまはREADMEで案内しています。
...
1と2は実装してあります。
3の`GetActivatedEventArgs()`はまだ呼んでおらず、通知を押してアプリが起動したとき（コールドローンチ）の分岐がありません。
[未決事項とリスク](open-questions.md)の「実装の残作業」に挙げてあります。

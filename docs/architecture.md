...
小窓はいま300×200のカードで固定です。
設計では、折りたたんだバー（およそ220×44）を既定の形にし、ホバーかクリックで300×200へ展開します。
この折りたたみはまだ実装していません（[未決事項とリスク](open-questions.md)の「実装の残作業」）。
既定の位置は主モニタの右上です。
右下はトーストの出現位置と衝突します。
...
ただし、全画面の窓がアプリ自身の掲示の窓なら、この判定に頼りません。
掲示中かどうかはアプリが知っているため、小窓を隠すこと、通知を止めること、更新を保留することに、その状態を直に使います。
掲示の窓の構成は[掲示モード](display.md)にあります。

クリックスルーは既定でOFFにします。
220×44なら入力の邪魔になりにくく、ホバーでの展開も使えます。
折りたたみを入れるときは、ONにするとホバーでの展開が使えなくなる旨を、設定画面に明記してください。
...
利用者から見えない位置に入るため、初回の起動時に案内します。

案内はトーストで1回だけ出し、「タスクバーの設定を開く」のボタンで`ms-settings:taskbar`を開きます。
項目の名前はWindowsの版で変わるため、ページだけを開きます。
出すかどうかの判断は`TrayGuide.Decide`（Core）にあります。

- **ダイアログにはしません。** 無人の端末、掲示の端末、自動の更新のあとの起動し直しで、OKを待って止まるためです
- 掲示で始めたときは出さず、次に掲示なしで起動したときへ回します
- すでに表へ出していれば出しません。`HKCU\Control Panel\NotifyIconSettings`の下にアイコンごとのキーがあり、表へ出すと`IsPromoted`が1になります。**文書化されていない場所**のため、読めなければ出ていないとみなします
- アプリの設定でトースト通知を切っている利用者には出しません
- トーストを出せなければトレイのバルーンへ倒します。どちらでも、出したら済んだと`settings.json`に記録します
...
アプリが終わっているときに通知を押すと、Windowsがアプリを起動します（コールドローンチ）。
動いているときに押されたのと同じ扱い（`ShowRequest.Notification`）にします。

**見分けるのは起動の引数です。**
Windows App SDKは、通知のCOMサーバーの起動の行として`"exe" ----AppNotificationActivated:`を登録します。
実際の起動では、そのあとにCOMの付ける`-Embedding`も続きます。
引数はUIのスレッドで待たずに読めるため、判断はこれで決めます（`NotificationLaunch.Classify`）。

活性化の種類は、診断の表示のためだけに別のスレッドで読みます。

Microsoft Learnの本文とサンプルコードの食い違いは、実機（2026年9月19日、Windows 11 25H2）で確かめました。
トーストを押したときにWindowsが送るのと同じCOMの呼び出し（`INotificationActivationCallback.Activate`）を、外から送りました。
`Register()`のあとの`GetActivatedEventArgs()`は`AppNotification`を返しました。
サンプルコードの側と一致し、本文の`Launch`とは違いました。
通知センターで実際に押しては確かめていません。

**`GetActivatedEventArgs()`はUIのスレッドで呼びません。**
通知の引数を持って起動されたのにCOMの呼び出しが来ないと、2秒ほど待ってから`TimeoutException`（`0x800705B4`）を投げます。
最初の実装はこれを捕まえ損ね、起動の途中で落ちました。
捕まえても、UIのスレッドで呼べばそのあいだ起動が止まります。
登録していないときも読みません。
COMの呼び出しを受ける口が無く、待つだけになるためです。
...
掲示モードのあいだは、逆に状態を進めます。
掲示が同じ判定を画面に出し続けており、利用者はそれを受け取っているためです。
止めた通知は、次の「出せなかった通知の落とし方」も通しません（[掲示モード](display.md)）。
...
| ランタイムの連鎖インストール | 終了コード0。すでに新しい版がある端末でも通ります。このとき同梱していたのは、のちに誤りと分かった1.8系です（[技術選定](stack.md)の「配布サイズ」） |
